...
public class TileLoader {
    ...
    // Tile Provider от Yandex MapKit
    ...
    public boolean initialize() {
        ...
        try {
            ...
            // Получаем Tile Provider от Yandex MapKit
            MapKit mapKit = MapKitFactory.getInstance();
            if (mapKit != null) {
                // Создаем Tile Provider для векторных тайлов
    ...
    public void loadTileDirect(int tileX, int tileY, int zoom, TileLoadCallback callback) {
        ...
        mTileLoaderExecutor.execute(new TileLoadDirectTask(tileX, tileY, zoom, callback));
    }
        ...
        public void run() {
            try {
                if (mTileProvider != null) {
                    // Получаем тайл через Tile Provider
                    // TODO: использовать реальный API Tile Provider
                    // Пока используем прямой HTTP загрузку
                    loadTileDirect(mTileX, mTileY, mZoom, mCallback);
                } else {
                    // Если Tile Provider недоступен, используем прямой HTTP загрузку
                    loadTileDirect(mTileX, mTileY, mZoom, mCallback);
                }
            } catch (Exception e) {
