...
public class TiggoSecondaryRenderThread extends Thread {
    ...
    public synchronized void setSurface(Surface surface) {
        ...
        // НЕ создаем окно здесь - оно будет создано в run() после создания EGL Surface
    ...
    private void createNativeWindow() {
        if (mIndex >= 0) {
            // Окно уже создано
            Log.w(TAG, "Native window already created, index=" + mIndex);
            return;
        }
        
        if (mEglSurface == null || mEglSurface == EGL10.EGL_NO_SURFACE) {
            Log.e(TAG, "Cannot create native window: EGL Surface not ready");
            return;
        }
        
        Log.i(TAG, "Creating native window: w=" + mWidth + ", h=" + mHeight + ", simplified=" + mSimplified);
        
        // Создаем окно в нативном коде
        // ВАЖНО: AddSecondaryWndGL уже вызывает CreateSecondaryGL внутри, поэтому
        // не нужно вызывать CreateSecondaryGL отдельно
        ...
        if (mIndex >= 0) {
            ...
            // AddSecondaryWndGL уже создал окно и инициализировал GL контекст внутри
            // Просто помечаем как инициализированное
            mRenderInitialized = true;
            Log.i(TAG, "TiggoSecondaryRenderThread():RenderGL initialized successfully, index=" + mIndex);
        } else {
            Log.e(TAG, "TiggoSecondaryRenderThread():Failed to create native window, AddSecondaryWndGL returned " + mIndex);
    ...
    public void run() {
        ...
        // Создаем окно в нативном коде (после создания EGL Surface)
        // Это должно быть сделано после eglMakeCurrent, чтобы нативный код мог использовать OpenGL
        if (mIndex < 0) {
            Log.i(TAG, "Creating native window after EGL surface creation");
            createNativeWindow();
        }
        
        if (mRenderInitialized && mIndex >= 0) {
            Log.i(TAG, "TiggoSecondaryRenderThread():RenderGL initialized, index=" + mIndex);
        } else {
            Log.e(TAG, "TiggoSecondaryRenderThread():Failed to initialize render, index=" + mIndex + ", initialized=" + mRenderInitialized);
        }
        ...
        while (mRunning) {
            ...
            if (mRenderInitialized && mIndex >= 0 && mEglSurface != null && mEglSurface != EGL10.EGL_NO_SURFACE) {
                // Убеждаемся, что контекст активен
                if (!mEgl.eglMakeCurrent(mEglDisplay, mEglSurface, mEglSurface, mEglContext)) {
                    Log.e(TAG, "eglMakeCurrent failed in render loop");
                    continue;
                }
                ...
                if (!mRenderInitialized) {
                    Log.w(TAG, "Render not initialized yet");
                }
                if (mIndex < 0) {
                    Log.w(TAG, "Window index invalid: " + mIndex);
                }
                if (mEglSurface == null || mEglSurface == EGL10.EGL_NO_SURFACE) {
                    Log.w(TAG, "EGL Surface invalid");
                }
            }
